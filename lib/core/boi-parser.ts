...
// Next.js. Leaving it unset uses the built-in main-thread fallback.
...
function groupLines(tokens: Token[]): Line[] {
  ...
  if (current.length)
    lines.push({ y: cy as number, tokens: [...current].sort((a, b) => a.x0 - b.x0) })
  return lines
}
...
function detectAnchors(lines: Line[], width: number): { anchors: Anchors; headerY: number | null } {
  ...
  if (
    headerEdge.out === undefined ||
    headerEdge.in === undefined ||
    headerEdge.balance === undefined
  ) {
  ...
  for (const line of lines) {
    ...
    for (const t of line.tokens) {
      ...
      for (const c of cols) {
        if (
          Math.abs(t.x1 - (headerEdge[c] as number)) < Math.abs(t.x1 - (headerEdge[best] as number))
        )
          best = c
      }
...
function classifyAmount(
  ...
  let best: Column = cols[0]
  for (const c of cols) {
    if (Math.abs(t.x1 - (anchors[c] as number)) < Math.abs(t.x1 - (anchors[best] as number)))
      best = c
  }
  ...
  for (const { tokens, width } of pages) {
    ...
    for (const line of lines) {
      ...
      for (const t of line.tokens) {
        ...
        else if (t.text === "OD" && t.x0 > balEdge - OD_MARGIN)
          continue // overdraft marker
